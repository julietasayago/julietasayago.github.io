...
export function useScrollAnimations({
  ...
  useEffect(() => {
    ...
    // Only set on the desktop branch below. Anchor clicks use it when
    // present and fall back to native smooth-scrolling on mobile/tablet,
    // where there's no Lenis instance smoothing the scroll position.
    ...
    const ctx = gsap.context(() => {
      ...
      mm.add({ isDesktop: DESKTOP_QUERY, isMobile: MOBILE_QUERY }, (context) => {
        ...
        if (isDesktop) {
          ...
          const dist = () =>
            track ? Math.max(0, track.scrollWidth - window.innerWidth + 96) : 0;
